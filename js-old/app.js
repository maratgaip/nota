...
jQuery(window).bind("lalaSongUnLoved", AudioPlayer.List.unLove);
jQuery(window).bind("lalaQueueSong", AudioPlayer.List.add);
jQuery(window).bind("lalaDownloadSong", DownloadSongCollection.url);
jQuery(document).ready(AudioPlayer.Init);
if (typeof (History) == "undefined") {
    History = {}
}
...
History.Anchor = {
    click: function (c) {
        var a = jQuery(this).attr("href");
//        var iddownload = jQuery(this).attr("id");
//        if(iddownload == "download_song"){
//            return DownloadSongCollection.url
//        }
        //jQuery("#download_song").live("click", );
        if (a) {
            if (jQuery(this).attr("target") != "_blank") {
                if (jQuery(this).parent("li").hasClass("dropped") == false) {
                    if (History.UseHistoryAPI == true) {
