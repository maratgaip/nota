...
HTML;
	...
	if($q){
		$db->query("SELECT * FROM vass_artists WHERE name LIKE '%$q%' LIMIT $start,20");
	}else{
	...
	$artist_list .= "
			...
			<div class=\"media-body\">
                ...
                <ul class=\"dropdown-menu\">
                  <li><a href=\"{$PHP_SELF}?do=videos&artist_id={$row['id']}\">Videos</a></li>
	...
	if($q){
		$total = $db->super_query("SELECT COUNT(*) AS count FROM vass_artists WHERE name LIKE '%$q%'");
		$pages = navigation("admin/index.php?do=artists&q=" . urlencode($q) . "&p={page}", $total['count'], 20);
	}else{
	...
	<form class="form-search" mothod="GET" action="{$PHP_SELF}index.php">
		<input name="do" type="hidden" value="artists">
