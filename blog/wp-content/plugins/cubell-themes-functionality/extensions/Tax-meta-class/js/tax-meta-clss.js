...
jQuery(document).ready(function($) {
  ...
  */
    ...
    jQuery(".mupload_img_holder").each(function(i,v){
      if (jQuery(this).next().next().val() != ''){
        if (!jQuery(this).children().size() > 0){
          jQuery(this).append('<img src="' + jQuery(this).next().next().val() + '" style="height: 150px;width: auto;" />');
  ...
  jQuery('.at-delete_image_button').live('click', function(e){
    ...
    $('.at-upload_image_button').closest('.form-field').before('<h3 id="cb-subhead">Background Options</h3>');
  //upload button
  var formfield1;
  var formfield2;
