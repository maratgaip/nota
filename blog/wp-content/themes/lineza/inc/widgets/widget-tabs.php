...
class bpxl_tabs extends WP_Widget {
	...
	function bpxl_tabs() {
		...
		$this->WP_Widget( 'tabs_widget', __('Lineza Tabs Widget', 'bloompixel'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<!-- START WIDGET -->
		<div id="tabs-widget">
			<ul id="tabs" <?php if ( $heading_background != 'default-bg' ) { echo 'class="' . $heading_background . '"'; } ?>>
			...
			<div id="tabs-content">
				<div id="tab1" class="popular-posts tab-content" style="display: block;">
					<ul>
						<?php
							$t_count = 1;
							$popularposts = new WP_Query('showposts='.$recent_posts_count.'&orderby=comment_count');
						?>
								...
								<?php if ( $show_thumb == 1 ) { ?>
									<div class="thumbnail">
										<a class="widgetthumb" href='<?php the_permalink(); ?>'>
											...
											<?php else: ?>
												<img src="<?php echo get_template_directory_uri(); ?>/images/100x100.png" alt="<?php the_title(); ?>"  width='100' height='100' class="wp-post-image" />
											<?php endif; ?>
											<span class="post-number"><?php echo $t_count; ?></span>
										</a>
									</div>
								<?php } ?>
								<div class="info">
				...
				<div id="tab2" class="recent-posts tab-content">
					<ul>
						<?php 
							$t_count = 1;
							$the_query = new WP_Query( 'showposts='.$recent_posts_count );
